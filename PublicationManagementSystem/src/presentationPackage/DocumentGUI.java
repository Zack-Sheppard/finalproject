...
public class DocumentGUI {
	...
	private void initialize() {
		frame = new JFrame();
		frame.setBounds(100, 100, 396, 478);
		frame.setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
		frame.getContentPane().setLayout(null);
		
		textField = new JTextField(doc.getTitle());
		textField.setBounds(135, 31, 217, 22);
		frame.getContentPane().add(textField);
		textField.setColumns(10);
		
		textField_1 = new JTextField(doc.getAuthor());
		textField_1.setColumns(10);
		textField_1.setBounds(135, 76, 217, 22);
		frame.getContentPane().add(textField_1);
		
		textField_2 = new JTextField(Double.toString(doc.getPrice()));
		textField_2.setColumns(10);
		textField_2.setBounds(135, 123, 217, 22);
		frame.getContentPane().add(textField_2);
		
		textField_3 = new JTextField(doc.getPublisher());
		textField_3.setColumns(10);
		textField_3.setBounds(135, 174, 217, 22);
		frame.getContentPane().add(textField_3);
		
		textField_4 = new JTextField(doc.getType());
		textField_4.setColumns(10);
		textField_4.setBounds(135, 225, 217, 22);
		frame.getContentPane().add(textField_4);
		
		textField_5 = new JTextField(doc.getGenre());
		...
		btnFinalize.setBounds(217, 373, 97, 25);
		...
		btnCancel.setBounds(77, 373, 97, 25);
		frame.getContentPane().add(btnCancel);
		
		chckbxOnSale = new JCheckBox("      On Sale");
		chckbxOnSale.setEnabled(doc.isOnSale());
		chckbxOnSale.setBounds(188, 319, 113, 25);
		frame.getContentPane().add(chckbxOnSale);
	}
	
	public void finish() {
		// Get info from JTextFields, construct Doc
		Document doc = new Document(textField.getText(), chckbxOnSale.isEnabled(), 
										Double.parseDouble(textField_2.getText()), 
										textField_3.getText(), textField_4.getText(), 
										textField_5.getText(), textField_1.getText());
		// If ID > 0 (editing), delete the doc, then add the edited version
		if(docID > 0) {
			op.removeDoc(docID);
		}
		// Add the doc
